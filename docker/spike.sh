...
has_mono() { wine uninstaller --list 2>/dev/null | grep -qi "wine mono" || ls "$WINEPREFIX/drive_c/windows/mono" >/dev/null 2>&1 || grep -qi "wine-mono" "$WINEPREFIX/system.reg" 2>/dev/null; }
...
if timeout 120 script -qfec "xvfb-run -a wine $(winpath "$APTIV") --help" "$OUT/aptiv_help.txt" >/dev/null 2>&1; grep -q -- "--asc" "$OUT/aptiv_help.txt"; then
...
step "판정 요약"
for k in dotnet aptiv_help aptiv_convert engine djlp_help djlp_convert; do [ -n "${R[$k]:-}" ] && printf "  %-14s %s\n" "$k" "${R[$k]}"; done
echo "산출물·로그: $OUT"
